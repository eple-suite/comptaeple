...
export function RapportOrdoSection() {
  ...
  // Répartition dépenses par nature
  const chargesNature = R.chargesNature ?? {};
  const depNatureData = useMemo(() => {
    const labels: Record<string, string> = {
      '60': 'Achats', '61': 'Serv. extérieurs', '62': 'Autres serv. ext.', '63': 'Impôts',
      '64': 'Personnel', '65': 'Autres charges', '66': 'Charges fin.', '67': 'Charges except.',
      '68': 'Dotations amort.', '20': 'Investissement', '21': 'Immobilisations',
    };
    const colors = ['hsl(215,70%,50%)', 'hsl(160,45%,45%)', 'hsl(38,92%,50%)', 'hsl(0,72%,55%)', 'hsl(280,50%,50%)', 'hsl(190,60%,40%)', 'hsl(340,65%,50%)', 'hsl(120,40%,40%)', 'hsl(30,70%,50%)', 'hsl(250,50%,50%)'];
    return Object.entries(chargesNature)
      .filter(([, v]) => v > 50)
      .sort(([, a], [, b]) => b - a)
      .slice(0, 10)
      .map(([k, v], i) => ({ name: labels[k] || `Cpt ${k}`, value: v, fill: colors[i % colors.length] }));
  }, [chargesNature]);

  // Répartition recettes par origine (État/Collectivité/Propres)
  const recettesOrigineData = useMemo(() => {
    const produitsOrigine = R.produitsOrigine ?? {};
    let etat = 0, collectivite = 0, propres = 0, autres = 0;
    Object.entries(produitsOrigine).forEach(([k, v]) => {
      if (['741', '744', '745', '746'].some(p => k.startsWith(p))) etat += v;
      else if (['742', '743', '747'].some(p => k.startsWith(p))) collectivite += v;
      else if (['70', '71', '72', '75', '76'].some(p => k.startsWith(p))) propres += v;
      else autres += v;
  ...
  // FDR en jours avec seuil 30 jours
  const fdrJoursGaugeData = [
    { name: 'FDR', jours: Math.round(safe.joursFdr), fill: safe.joursFdr >= 30 ? 'hsl(160,45%,45%)' : safe.joursFdr >= 15 ? 'hsl(38,92%,50%)' : 'hsl(0,72%,55%)' },
  ];
  ...
  // ── Smart auto-commentary for CA members ──────────────────
  const autoComments = useMemo(() => {
    ...
    if (R.resultatBudgetaire >= 0) {
      ...
      const prelev = safe.prelevementsReserves.totalPrelevements;
      if (prelev > 0) {
        const horsP = R.resultatBudgetaire + prelev;
        c.push(`📊 Le résultat est déficitaire de ${formatEur(Math.abs(R.resultatBudgetaire))} en raison de ${formatEur(prelev)} de prélèvements sur fonds de roulement votés en CA. ${horsP >= 0 ? `Sans ces prélèvements, le résultat aurait été excédentaire de ${formatEur(horsP)}.` : `Hors prélèvements, le résultat reste déficitaire de ${formatEur(Math.abs(horsP))}, ce qui appelle une vigilance accrue.`}`);
      } else {
        c.push(`⚠️ Le résultat est déficitaire de ${formatEur(Math.abs(R.resultatBudgetaire))}. Ce déficit sera imputé sur les réserves (solde actuel : ${formatEur(R.reserves)}).`);
      }
    }
    ...
    // Taux d'exécution
    if (R.tauxExecCharges < 0.75) {
      c.push(`⚠️ Le taux d'exécution des dépenses est faible (${(R.tauxExecCharges * 100).toFixed(1)}%). Cela indique une sous-consommation significative des crédits qui pourrait traduire un manque de dynamisme budgétaire ou des retards dans les marchés.`);
    } else if (R.tauxExecCharges > 0.95) {
      c.push(`✅ Les crédits ont été consommés à ${(R.tauxExecCharges * 100).toFixed(1)}%, signe d'un pilotage budgétaire rigoureux.`);
    }

    // FDR jours
    if (safe.joursFdr >= 30) {
      c.push(`✅ Le fonds de roulement couvre ${Math.round(safe.joursFdr)} jours de fonctionnement (seuil recommandé : 30 jours). La situation financière est saine.`);
    } else if (safe.joursFdr > 0) {
      c.push(`🔴 Le fonds de roulement ne couvre que ${Math.round(safe.joursFdr)} jours de fonctionnement, en dessous du seuil recommandé de 30 jours. La marge de manœuvre est limitée.`);
    } else {
      c.push(`🔴 Le fonds de roulement est négatif (${formatEur(R.fdrComptable)}). L'établissement doit impérativement retrouver une capacité d'autofinancement.`);
    }

    // Trésorerie
    if (safe.joursTresorerie < 15) {
      c.push(`🔴 La trésorerie disponible (${formatEur(R.tresorerie)}) ne couvre que ${Math.round(safe.joursTresorerie)} jours. Risque de tensions de trésorerie à court terme.`);
    } else if (safe.joursTresorerie > 90) {
      c.push(`💚 La trésorerie (${formatEur(R.tresorerie)}, ${Math.round(safe.joursTresorerie)} jours) offre une marge confortable.`);
    }

    // CAF
    if (R.cafBudgetaire >= 0) {
      c.push(`✅ La capacité d'autofinancement de ${formatEur(R.cafBudgetaire)} traduit la capacité de l'établissement à financer ses investissements sur ses propres ressources.`);
    } else {
      c.push(`⚠️ L'insuffisance d'autofinancement de ${formatEur(Math.abs(R.cafBudgetaire))} indique que le fonctionnement courant ne dégage pas assez de marges pour autofinancer les investissements.`);
    }

    // SRH
    if (srhDomaine) {
      if (srhSolde >= 0) {
        c.push(`🍽️ Le service de restauration et d'hébergement (D5) dégage un excédent de ${formatEur(srhSolde)}. L'équilibre est assuré.`);
      } else {
        c.push(`🍽️ Le SRH (D5) est déficitaire de ${formatEur(Math.abs(srhSolde))}. Le coût de fonctionnement dépasse les recettes perçues, ce qui pèse sur le budget général.`);
      }
    }

    return c;
  }, [R, safe, etab, srhDomaine, srhSolde]);

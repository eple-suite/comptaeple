...
import { CheckCircle2, AlertTriangle, Ban, BookOpen } from 'lucide-react';
...
export function CheckListSection() {
  ...
  if (!R) return <EmptyState msg="Lancez l'analyse depuis la section Imports pour afficher la check-list." />;
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KPICard label="Contrôles OK" value={nbOK} color="green" icon="✅" sub={`Sur ${checkItems.length} vérifications`} />
      ...
      <Card>
        <CardHeader className="bg-gradient-to-r from-slate-800 to-slate-700 rounded-t-lg">
          <CardTitle className="text-white text-sm flex items-center gap-2">
            ...
            Check-List des rapprochements M9-6
            <span className="ml-auto text-slate-400 text-xs">Décret 2012-1246 — M9-6 2026</span>
          </CardTitle>
        </CardHeader>
...
function CheckRow({ check }: { check: CheckItem }) {
  const icons = { ok: <CheckCircle2 className="h-5 w-5 text-emerald-600" />, warn: <AlertTriangle className="h-5 w-5 text-warning" />, erreur: <AlertTriangle className="h-5 w-5 text-destructive" />, bloquant: <Ban className="h-5 w-5 text-destructive" /> };
  const rowBg = { ok: '', warn: 'bg-warning/5', erreur: 'bg-destructive/5', bloquant: 'bg-destructive/10 border-l-4 border-l-destructive' };
  const badgeVariant = { ok: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400', warn: 'bg-warning/20 text-warning', erreur: 'bg-destructive/20 text-destructive', bloquant: 'bg-destructive text-destructive-foreground' };
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-2">
            ...
            <Badge className={`text-xs ${badgeVariant[check.statut]}`}>
              {check.statut === 'ok' ? 'OK' : check.statut === 'bloquant' ? 'BLOQUANT' : 'ANOMALIE'}
            </Badge>
          ...
          {check.piste && (
            <div className="text-xs text-primary bg-primary/5 rounded px-2 py-1.5 mt-1.5">
              💡 <strong>Piste :</strong> {check.piste}
            </div>
          )}

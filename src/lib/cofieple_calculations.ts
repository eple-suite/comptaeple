...
function findCol(row: Record<string, string>, ...names: string[]): string {
  ...
  // Last resort: "endsWith" match — handles Op@le headers with
  // "Somme de" / "Affichage" prefixes, e.g.
  // "Somme de Affichage Solde débit" endsWith "solde debit" → match
  // Using endsWith (not includes) avoids "Montant débit" matching "Montant débit antérieur"
  for (const [key, val] of Object.entries(row)) {
    const nk = normalizeHeader(key);
    for (const nn of normalizedNames) {
      if (nn.length >= 4 && nk.endsWith(nn)) return String(val ?? '');
    }
  }
  return '';
}
...
export function parserSDR(rows: Record<string, string>[], _typeBudget: TypeBudget): LigneSDR[] {
  ...
  if (hasDirectCols) {
    return rows.map(r => enrichParsedSdrRow({
      ...
      realise: toNumDirect(findCol(r, 'réalisé', 'realise', 'Réalisé', 'Realise', 'REALISE', 'aor', 'AOR', 'émis', 'Émis', 'emis', 'Emis', 'titré', 'Titré', 'Montant net des recettes', 'montant net des recettes', 'Recettes nettes', 'recettes nettes')),
      encours: toNumDirect(findCol(r, 'en cours', 'encours', 'En cours')),
...
export function parserBalance(rows: Record<string, string>[], _typeBudget: TypeBudget): LigneBalance[] {
  ...
  if (hasDirectCols) {
    return rows
      ...
      .filter((r): r is LigneBalance => r != null);
  }
